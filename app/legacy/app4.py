...
def fig_bond_hyperbolic(alerts, defaults, K=0.35, alpha=0.55):
    ...
    ax = axes[0]
    d = (alerts.dropna(subset=["PD_3M", "Momentum"]).copy()
         if alerts is not None and not alerts.empty else pd.DataFrame())

    # Load lead-time first-alarm points so ALL defaulted issuers are plotted at their alarm peak
    try:
        con = sqlite3.connect(DB)
        blt = pd.read_sql("select * from bond_ews_leadtime", con)
        con.close()
        if not blt.empty:
            for _, lr in blt.iterrows():
                icode = str(lr["issuer_code"])
                if pd.notna(lr["PD_3M"]) and pd.notna(lr["Momentum"]):
                    d = pd.concat([d, pd.DataFrame([{
                        "issuer_code": icode,
                        "PD_3M": float(lr["PD_3M"]),
                        "Momentum": float(lr["Momentum"]),
                        "PD_prev": float(lr["PD_3M"]) / float(lr["Momentum"]) if float(lr["Momentum"]) > 0 else 0.001,
                        "alert": lr["alert"],
                    }])], ignore_index=True)
    except Exception:
        pass
    ...
    if d.empty:
        ...
        ax.fill_between(xs * 100, K / (xs ** alpha), yhi, color="#f97316", alpha=0.10,
                        label="flagged zone")
        ax.scatter(x[~flag], y[~flag], s=10, c="#86a89a", alpha=0.20, linewidths=0,
                   label=f"below boundary ({int((~flag).sum()):,})")
        ax.scatter(x[flag], y[flag], s=42, c="#dc2626", alpha=0.85, linewidths=0,
        ...
        for _, r in flagged_df.iterrows():
            ...
            if code:
                ...
                lbl = f"{code} ({n_def} defaults)" if n_def > 0 else code
                ax.annotate(lbl, (xi, yi), fontsize=8, fontweight="bold",
                            color="#991b1b", xytext=(5, 3), textcoords="offset points",
                            zorder=6)
        ax.set_xlim(0, xhi); ax.set_ylim(0.3, yhi)
...
def main(page):
    ...
    nav_styles = [
        ...
        ("#f0fdfa", "#0f766e", "#0f766e"), # Tab 16: XGBoost curve forecast (Teal)
        ("#eef2ff", "#4338ca", "#4338ca"), # Tab 17: iBond corporate bonds (Indigo)
    ]
    ...
    btn_tab16 = make_nav_button("XGBoost: Slope & Curvature Forecast", ft.Icons.INSIGHTS, 16)
    btn_tab17 = make_nav_button("Download ข้อมูลหุ้นกู้ & Lead Time", ft.Icons.CLOUD_DOWNLOAD, 17)
    btn_tab18 = make_nav_button("Download ข้อมูลหุ้นกู้ Approach 2", ft.Icons.EQUALIZER, 18)

    baseline_buttons = [make_nav_button(label, icon, idx=19+b_i) for b_i, (_, label, icon) in enumerate(BASELINE_MENUS)]
    ...
    nav_buttons = [btn_tab0, btn_tab1, btn_tab2, btn_tab3, btn_tab4, btn_tab5,
                   ...
                   btn_tab13_logs, btn_tab14, btn_tab15, btn_tab16, btn_tab17, btn_tab18] + baseline_buttons
    ...
    def update_bond_xgb_tab(_=None):
        ...
        r = bsum.iloc[0]
        bnd_xgb_verdict.value = f"Approach 2 (XGBoost Hazard) on iBond data — AUC out-of-sample {float(r['auc_oos']):.3f} | caught {int(r['n_caught'])}/{int(r['n_events'])} defaulted issuers | median lead time {float(r['median_lead_days']):.0f} days"
        ...
        bnd_xgb_kpis.controls = [
            kpi("🏆 OOS AUC", f"{float(r['auc_oos']):.3f}", "XGBoost hazard model", ft.Icons.EMOJI_EVENTS, C.GREEN_700, C.GREEN_50),
            kpi("🎯 Defaults caught", f"{int(r['n_caught'])} / {int(r['n_events'])}", "100% detection rate", ft.Icons.CHECK_CIRCLE, C.TEAL_700, C.TEAL_50),
            kpi("⏱️ Median lead time", f"{float(r['median_lead_days']):.0f} days", f"≈ {float(r['median_lead_days'])/30.44:.1f} months", ft.Icons.SCHEDULE, C.INDIGO_700, C.INDIGO_50),
            kpi("⚠️ High risk alerts", f"{int(r['n_high'])} issuers", "top risk quantile", ft.Icons.WARNING, C.ORANGE_800, C.ORANGE_50),
        ]
    ...
    def set_tab(idx):
        ...
        if idx >= 19:
            view, refresh = baseline_views[idx - 19]
            refresh()
    ...
    btn_tab18.on_click = lambda _: set_tab(18)
    for _i, _b in enumerate(baseline_buttons, start=19):
...
def _cli_run():
    ...
    if args.download_bonds or args.run_all_bonds:
        print("\n=== [1/3] Downloading Corporate Bond Universe & Defaults from iBond ===")
        import download_bond as dbnd
        dbnd.run(with_defaults=True, save=True, verbose=True)

    if args.approach1 or args.run_all_bonds:
        print("\n=== [2/3] Running Corporate Bond EWS Approach 1 (Cox Hazard Model) ===")
        import bond_ews as bews
        bews.run(refresh=False, save=True, verbose=True)

    if args.approach2 or args.run_all_bonds:
        print("\n=== [3/3] Running Corporate Bond EWS Approach 2 (XGBoost Hazard Model) ===")
        import bond_ews_xgb as bews_xgb
...
if __name__ == "__main__":
    if "--selftest" in sys.argv:
        ...
        _uitest()
    elif any(arg in sys.argv for arg in ["--approach1", "--run-a1", "--approach2", "--run-a2", "--download-bonds", "--run-all-bonds"]):
        _cli_run()
    else:
        import flet as ft
        ft.run(main)
