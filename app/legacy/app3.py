...
def fig_bond_hyperbolic(alerts, defaults, K=0.35, alpha=0.55):
    ...
    if d.empty:
        ...
        score = np.log(np.clip(y, 1e-9, None)) + alpha * np.log(np.clip(d["PD_3M"], 1e-9, None))
        flag = score.to_numpy() >= np.log(K)
        ...
        ax.fill_between(xs * 100, K / (xs ** alpha), yhi, color="#f97316", alpha=0.10,
                        label="flagged zone")
        ax.scatter(x[~flag], y[~flag], s=14, c="#86a89a", alpha=0.35, linewidths=0,
        ...
        ax.grid(alpha=0.25)
    ax.set_title("Hyperbolic alarm boundary — model panel", fontsize=10, loc="left")
...
def main(page):
    ...
    btn_tab17 = make_nav_button("iBond: Corporate Bonds & Defaults", ft.Icons.ACCOUNT_BALANCE, 17)

    baseline_buttons = [make_nav_button(label, icon, idx=18+b_i) for b_i, (_, label, icon) in enumerate(BASELINE_MENUS)]
    ...
    nav_buttons = [btn_tab0, btn_tab1, btn_tab2, btn_tab3, btn_tab4, btn_tab5,
                   ...
                   btn_tab13_logs, btn_tab14, btn_tab15, btn_tab16, btn_tab17] + baseline_buttons
    ...
    def update_bond_tab(_=None):
        ...
        bnd_info.value = (f"Source: iBond gRPC · last download "
                          ...
                          f"SQLite: ibond_issuer / ibond_corp_bond / ibond_default_payment")
    ...
    bond_card = card(ft.Column([
        ft.Row([ft.Icon(ft.Icons.ACCOUNT_BALANCE, color=C.INDIGO_700, size=22),
                ft.Text("🏦 Corporate bonds from iBond — issuers, universe, defaults",
                        size=15, weight=ft.FontWeight.BOLD, color=C.BLUE_900)], spacing=6),
        ft.Text("Pulls the issuer master, the registered bond universe and the payment-default "
                "register over gRPC-Web, then shows the hyperbolic alarm boundary beside the "
                "real default timeline.", size=11, color=C.GREY_700),
        ft.Row([
            ft.Button("Download from iBond", icon=ft.Icons.CLOUD_DOWNLOAD,
                      on_click=on_download_bond, bgcolor=C.INDIGO_700, color=C.WHITE,
                      style=ft.ButtonStyle(shape=ft.RoundedRectangleBorder(radius=8))),
            bnd_scan,
            ft.Button("Run Approach 1", icon=ft.Icons.ANALYTICS, on_click=lambda _: on_run_a1(),
                      bgcolor=C.RED_700, color=C.WHITE,
                      style=ft.ButtonStyle(shape=ft.RoundedRectangleBorder(radius=8))),
    ...
    def set_tab(idx):
        ...
        if idx >= 18:
            view, refresh = baseline_views[idx - 18]
            refresh()
    ...
    for _i, _b in enumerate(baseline_buttons, start=18):
