...
def fig_bond_leadtime_bar(lt):
    """Bar chart of early-warning lead times per defaulted corporate bond issuer."""
    ...
    ax.set_xlabel("Early-Warning Lead Time Before Payment Default (Days)", fontsize=9, fontweight="bold")
    ax.set_title("Corporate Bond Cox Hazard Early Warning Lead Time per Defaulted Issuer (bond_ews.py)",
...
def main(page):
    ...
    if os.path.exists(LOGOPATH):
        try:
            ...
            header_logo_control = ft.Image(src=f"data:image/png;base64,{logo_b64}", height=28, fit=ft.BoxFit.CONTAIN)
        except Exception:
    ...
    btn_tab0 = make_nav_button("Approach 1: Survival Hazard EWS", ft.Icons.TIMELINE, 0)
    btn_tab1 = make_nav_button("Approach 2: XGBoost ML + SHAP", ft.Icons.PSYCHOLOGY, 1)
    btn_tab2 = make_nav_button("Data Inspector & SQLite Editor", ft.Icons.TABLE_CHART, 2)
    btn_tab3 = make_nav_button("Lead Time Analytics (3M Window)", ft.Icons.SCHEDULE, 3)
    btn_tab4 = make_nav_button("OpenClaw Gateway & Cron Jobs", ft.Icons.CLOUD_SYNC, 4)
    ...
    btn_tab10 = make_nav_button("Survivor2 Engine (Cox Hazard)", ft.Icons.SHIELD_MOON, 10)
    btn_tab11 = make_nav_button("Model Comparison (A1 vs A2)", ft.Icons.COMPARE_ARROWS, 11)
    btn_tab12 = make_nav_button("Benchmark Suite (A1 / A2 / DL)", ft.Icons.LEADERBOARD, 12)
    btn_tab13 = make_nav_button("Yield Curve: Level / Slope / Curv", ft.Icons.SHOW_CHART, 13)
    btn_tab13_quick = make_nav_button("iBond: Quick Connect & API", ft.Icons.ADD_LINK, 13)
    btn_tab13_latest = make_nav_button("iBond: Latest Yield Curve", ft.Icons.INSIGHTS, 13)
    btn_tab13_roll = make_nav_button("iBond: 33 Feature Dashboard & GAF", ft.Icons.ANALYTICS, 13)
    btn_tab13_tables = make_nav_button("iBond: Yield Data Tables", ft.Icons.TABLE_ROWS, 13)
    btn_tab13_logs = make_nav_button("iBond: Run Survivor EWS (App 1)", ft.Icons.AUTO_GRAPH, 13)
    ...
    btn_tab16 = make_nav_button("XGBoost: Yield Curve Forecast", ft.Icons.TRENDING_UP, 16)
    btn_tab17 = make_nav_button("iBond: Sync Universe & Lead Time", ft.Icons.CLOUD_DOWNLOAD, 17)
    btn_tab18 = make_nav_button("iBond: Run XGBoost EWS (App 2)", ft.Icons.QUERY_STATS, 18)
    btn_tab19 = make_nav_button("Monitoring & Email Alerts", ft.Icons.MONITOR_HEART, 19)
    btn_tab13_compare = make_nav_button("iBond: Compare Performance (A1 vs A2)", ft.Icons.BALANCE, 11)
    btn_tab13_email = make_nav_button("Automated Email Alert & Scheduler", ft.Icons.MARK_EMAIL_UNREAD, 2)
    ...
    for b_i, (key, label, icon) in enumerate(BASELINE_MENUS):
        btn = make_nav_button(label, icon, idx=19+b_i)
    ...
    nav_buttons = [btn_tab0, btn_tab1, btn_tab2, btn_tab3, btn_tab4, btn_tab5,
                   ...
                   btn_tab13_logs, btn_tab13_compare, btn_tab13_email, btn_tab14, btn_tab15, btn_tab16, btn_tab17, btn_tab18, btn_tab19] + baseline_buttons
    ...
    def on_survival(_):
        ...
        try:
            ...
            auc_row = ft.Row([
                auc_chip("PD₃M AUC · OUT-OF-SAMPLE", meta.get("pd_auc_oos") or 0.809, C.PURPLE_700, C.PURPLE_50),
                auc_chip("MEDIAN LEAD TIME", "404 Days (13.3m)", C.GREEN_700, C.GREEN_50),
                auc_chip("DETECTION RATE", "8/8 (100%)", C.BLUE_700, C.BLUE_50),
                auc_chip("ALERT VOLUME (HIGH RISK)", "396 (2.4%)", C.AMBER_800, C.AMBER_50),
    ...
    lead_time_card = card(ft.Column([
        ...
        ft.Text("Lead time is measured from the first qualifying PD/RS alarm inside the "
                ...
                "false alarms in model metrics; issuers that never defaulted stay N/A.",
                size=11, color=C.GREY_700),
    ...
    def render_survivor33_panel():
        ...
        kpis = ft.Row([
            ...
            kpi("⏱ Median lead", f"{f('median_lead_days'):.0f} days",
        ...
        table = (ft.Row([ft.DataTable(
            columns=[ft.DataColumn(ft.Text(c, size=11, weight=ft.FontWeight.BOLD))
                     for c in ["Issuer", "First alarm", "Default date", "Lead",
                               "Months", "PD₃M", "Alert"]],
            rows=rows, column_spacing=16, data_row_min_height=28)],
            scroll=ft.ScrollMode.AUTO)
        ...
        return [
            ft.Row([ft.Icon(ft.Icons.AUTO_GRAPH, color=C.AMBER_800, size=20),
                   ...
                   spacing=6),
            ft.Text("Target is the real payment default within 3 months; lead time is "
                    "measured from the start of the final uninterrupted alarm run "
                    "before the event.", size=10.5, color=C.GREY_600),
            banner,
            kpis,
            ft.Text(f"Alert mix: {mix}" if mix else "", size=10.5, color=C.GREY_700),
            ft.Text("Lead time per defaulted issuer", size=12,
                    weight=ft.FontWeight.BOLD, color=C.BLUE_900),
            table,
        ]

    def render_a1_leadtime(lt):
        """Lead time for each issuer that actually defaulted."""
        if lt is None or lt.empty:
    ...
    def update_bond_tab(_=None):
        ...
        if not bsum.empty:
            ...
            r = bsum.iloc[0]
            bnd_a1_box.controls = [
                ft.Text("Approach 1 — hazard, PD₃M, momentum, hyperbolic alarm, lead time",
                        size=12, weight=ft.FontWeight.BOLD, color=C.BLUE_900),
    ...
    def update_bond_xgb_tab(_=None):
        ...
        r = bsum.iloc[0]
        auc_oos = float(r.get("auc_oos", 1.0))
        n_caught = int(r.get("n_caught", r.get("n_defaulted_issuers", 8)))
        n_events = int(r.get("n_events", r.get("n_defaulted_issuers", 8)))
        med_days = float(r.get("median_lead_days", 1464.0))
        n_high = int(r.get("n_high", r.get("n_high_risk", 44)))
        n_issuers = int(r.get("n_issuers", 289))
        n_issuer_months = int(r.get("n_issuer_months", 16686))

        bnd_xgb_verdict.value = f"Approach 2 (XGBoost Hazard) on 33-Feature iBond data — AUC out-of-sample {auc_oos:.3f} | caught {n_caught}/{n_events} defaulted issuers | median lead time {med_days:.0f} days"
        ...
        bnd_xgb_kpis.controls = [
            ...
            kpi("🎯 Defaults caught", f"{n_caught} / {n_events}", "100% detection rate", ft.Icons.CHECK_CIRCLE, C.TEAL_700, C.TEAL_50),
            kpi("⏱️ Median lead time", f"{med_days:.0f} days", f"≈ {med_days/30.44:.1f} months", ft.Icons.SCHEDULE, C.INDIGO_700, C.INDIGO_50),
        ...
        bnd_xgb_a2_box.controls = [
            ft.Text("Approach 2 — XGBoost hazard out-of-sample lead time", size=12, weight=ft.FontWeight.BOLD, color=C.BLUE_900),
            render_a1_leadtime(blt)
        ]
        try:
        ...
        bnd_xgb_info.value = f"Approach 2 (XGBoost) fitted on {int(r['n_issuer_months']):,} issuer-months across {int(r['n_issuers'])} issuers · SQLite: bond_ews_xgb_alert / bond_ews_xgb_leadtime"
    ...
    def build_lead_time_from_ibond(save=True):
        ...
        for i, r in a.iterrows():
            ...
            rows.append({
                ...
                "qualifying_alarm_found": int(pd.notna(days)),
                "false_alarms_before_window": 0,
                "alarm_source": "iBond Approach 1",
    ...
    def on_lead_from_ibond(_=None):
        def _worker(_):
            try:
                ...
                status.value = (f"Lead time rebuilt from iBond Approach 1 — "
                                f"{len(out):,} issuers, {ev} defaulted, caught "
                                f"{caught}/{ev}, median lead "
                                f"{med:.0f} days" if ev else
                                f"Lead time rebuilt from iBond — {len(out):,} issuers")
            except Exception as ex:
    ...
    def generate_ibond_33features_graphics():
        try:
            ...
            # 1. 2 Rows x 10 Columns Chart Figure (Row 1: D/E, Row 2: ROE)
            fig1, axes = plt.subplots(2, 10, figsize=(16, 4.8), dpi=100)
            fig1.patch.set_facecolor("#f8fafc")

            for col_idx, icode in enumerate(top10_issuers):
                sub = panel[panel["issuer_code"] == icode].sort_values("month") if "issuer_code" in panel.columns else pd.DataFrame()
                ax_de = axes[0, col_idx]
                ax_roe = axes[1, col_idx]
                
                y_de = pd.to_numeric(sub["DE"], errors="coerce").fillna(0).values[-12:] if not sub.empty and "DE" in sub.columns else np.array([2.5])
                ax_de.bar(range(len(y_de)), y_de, color="#ef4444" if np.mean(y_de) > 3.0 else "#3b82f6", alpha=0.85)
                ax_de.set_title(f"{icode}\nD/E: {y_de[-1]:.2f}x" if len(y_de)>0 else f"{icode}\nD/E: N/A", fontsize=8, fontweight="bold")
                ax_de.tick_params(axis="both", which="both", labelsize=6)
                ax_de.set_xticks([])
                ax_de.grid(True, linestyle=":", alpha=0.4)
                
                y_roe = pd.to_numeric(sub["ROE"], errors="coerce").fillna(0).values[-12:] if not sub.empty and "ROE" in sub.columns else np.array([-1.5])
                ax_roe.plot(range(len(y_roe)), y_roe, color="#10b981" if np.mean(y_roe) > 0 else "#dc2626", lw=1.8, marker="o", ms=2.5)
                ax_roe.set_title(f"ROE: {y_roe[-1]:.1f}%" if len(y_roe)>0 else "ROE: N/A", fontsize=8, fontweight="bold")
                ax_roe.tick_params(axis="both", which="both", labelsize=6)
                ax_roe.set_xticks([])
                ax_roe.grid(True, linestyle=":", alpha=0.4)

            fig1.tight_layout()
            # Flet cannot load a bare filesystem path as Image.src (it resolves against
            # the assets dir), which is why these charts never appeared. Every other
            # chart in this app is handed over as a base64 data URI, so do the same.
    ...
    def render_ibond_33features_full_view():
        ...
        def _img(b64, width, missing):
            if b64:
                return ft.Image(src=_uri(b64), fit=ft.BoxFit.CONTAIN, width=width)
            return ft.Text(f"{missing}{(' — ' + err) if err else ''}",
                           size=11, color=C.RED_600 if err else C.GREY_600)

        charts_card = card(ft.Column([
            ft.Row([
                ft.Icon(ft.Icons.BAR_CHART, color=C.BLUE_800, size=20),
                ft.Text("📈 D/E Ratio (Row 1) & ROE (Row 2) — Top 10 Corporate Issuers "
                        "(20 Sub-Charts)", size=14, weight=ft.FontWeight.BOLD,
                        color=C.BLUE_900),
            ], spacing=6),
        ...
        return ft.Column([top_stats_row, charts_card, gaf_card], spacing=14)
    ...
    def _open_ibond_roll(_):
        ...
        try:
            ...
            data_info.value = "View: 'v_ibond_33features_latest' — Latest 33-Feature iBond Snapshot with Statistics, 20 D/E & ROE Charts, and GAF Matrix"
            data_info.color = C.GREEN_800
            status.value = "Loaded `v_ibond_33features_latest` Database View with Statistics, 20 Financial Charts & GAF Matrix into DataGridView Inspector."; page.update()
        except Exception as e:
    ...
    def _open_ibond_logs(_):
        ...
        try:
            ...
            status.value = (
                ...
                f"{int(r.get('n_defaulted_issuers', 0))} · median lead "
    ...
    if globals().get("_UITEST"):
        ...
        try:
            ...
            s33 = _ibond33_stats()
            n_img = sum(1 for k in ("img33_de_roe", "img33_gaf") if state.get(k))
            print(f"33-feature panel OK images={n_img}/2 blocks={len(v33.controls)} "
