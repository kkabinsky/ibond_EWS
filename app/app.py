...
def _b64(fig, dpi=120):
    buf = io.BytesIO(); fig.savefig(buf, format="png", dpi=dpi, bbox_inches="tight", pad_inches=0.08)
...
def fig_alert_dist(alerts):
    ...
    for i, value in enumerate(cnt.values):
        ax.text(
            i,
            value,
            f"{int(value):,}",
...
def _is_ibond_33_table(table_name):
    name = str(table_name or "").strip().lower()
    return (
        name == "ibond_33features_panel"
        or name.startswith("v_ibond_33features_")
    )


def normalize_ews_panel(df):
    """Normalize identifiers, observation dates and true event onsets.

    In particular, `y_fwd` is a forward-looking label and must never be copied
    into `event`. The survival engine needs the real event-onset month.
    """
    if df is None or df.empty:
        return df

    out = df.copy()
    if "firm_id" not in out.columns and "issuer_code" in out.columns:
        out["firm_id"] = out["issuer_code"].astype(str)

    if "month_year" in out.columns:
        out["month_year"] = pd.to_datetime(
            out["month_year"], errors="coerce"
        )
    else:
        observation_col = next(
            (
                c for c in ("month", "as_of_date", "date")
                if c in out.columns
            ),
            None,
        )
        if observation_col:
            out["month_year"] = pd.to_datetime(
                out[observation_col], errors="coerce"
            )

    if "month_year" in out.columns and out["month_year"].notna().any():
        month_ordinal = (
            out["month_year"].dt.year * 12 + out["month_year"].dt.month
        )
        out["month_index"] = month_ordinal - month_ordinal.min() + 1
    elif "month_index" in out.columns:
        out["month_index"] = pd.to_numeric(
            out["month_index"], errors="coerce"
        )
    ...
    if "event" not in out.columns:
        event_col = next(
            (
                c for c in (
                    "default_event", "observed_default",
        ...
        out["event"] = (
            pd.to_numeric(out[event_col], errors="coerce").fillna(0)
            if event_col
            else 0
        )
    out["event"] = (
        pd.to_numeric(out["event"], errors="coerce")
        .fillna(0)
        .gt(0)
        .astype(int)
    )
...
def load_panel(prefer_33=True):
    con = sqlite3.connect(DB)
    try:
        c = con.cursor()
        c.execute("SELECT count(1) FROM sqlite_master WHERE type='table' AND name='ibond_33features_panel'")
        has_33 = prefer_33 and c.fetchone()[0] > 0
        if has_33:
            c.execute("SELECT count(1) FROM ibond_33features_panel")
            n_33 = c.fetchone()[0]
            if n_33 > 0:
                df = pd.read_sql_query("SELECT * FROM ibond_33features_panel", con)
            else:
...
def _prepare_boundary_plot(df, meta):
    """Return validated plot data in probability units (0-1), not percent."""
    acct = "firm_id" if "firm_id" in df.columns else ("ticker" if "ticker" in df.columns else "account_id")
    required = {"PD_prev", "Momentum", "y_fwd", acct}
    missing = required.difference(df.columns)
    if missing:
        raise ValueError(
            "boundary plot is missing columns: " + ", ".join(sorted(missing))
        )

    d = df.copy()
    for column in ("PD_prev", "Momentum", "y_fwd"):
        d[column] = pd.to_numeric(d[column], errors="coerce")
    d = d.dropna(subset=["PD_prev", "Momentum", "y_fwd"])
    d = d[
        d["PD_prev"].between(1e-12, 1.0)
        & d["Momentum"].gt(0)
        & d["Momentum"].lt(20)
    ].copy()
    if d.empty:
        raise ValueError("no finite PD/momentum observations for boundary plot")

    boundary = meta.get("boundary") or {}
    a = float(boundary.get("alpha", np.nan))
    K = float(boundary.get("K", np.nan))
    if not np.isfinite(a) or not np.isfinite(K) or a <= 0 or K <= 0:
        raise ValueError("invalid hyperbolic boundary parameters")
...
def boundary_plot_diagnostics(df, meta):
    """Small testable summary used to guard the boundary visual."""
    d, acct, a, K = _prepare_boundary_plot(df, meta)
    distress = d["y_fwd"].eq(1)
    flagged = d["_flag"] & ~distress
    normal = ~d["_flag"] & ~distress
    x = d["PD_prev"].to_numpy(dtype=float) * 100
    xhi = min(
        100.0,
        max(float(np.nanpercentile(x, 99.8)), float(np.nanmax(x)), 5.0),
    )
    right_curve = K / max(xhi / 100.0, 1e-6) ** a
    return {
        "account_column": acct,
        "normal_rows": int(normal.sum()),
        "flagged_rows": int(flagged.sum()),
        "distress_rows": int(distress.sum()),
        "flagged_firms": int(d.loc[d["_flag"], acct].nunique()),
        "x_max_percent": xhi,
        "right_edge_boundary": float(right_curve),
    }


def fig_boundary(df, meta, max_pts=4500, label_top=35):
    """Hyperbolic boundary with mutually exclusive green/orange/red classes."""
    d, acct, a, K = _prepare_boundary_plot(df, meta)
    flagged_firms = int(d.loc[d["_flag"], acct].nunique())
    flagged_months = int(d["_flag"].sum())

    # Keep all boundary/event rows and sample only the normal population.
    special = d[d["_flag"] | d["y_fwd"].eq(1)]
    rest = d[~d["_flag"] & ~d["y_fwd"].eq(1)]
    if len(rest) > max_pts:
    ...
    plt.subplots_adjust(left=0.08, right=0.96, top=0.92, bottom=0.12)          # ~3x the old single-panel area
    xhi = min(100.0, max(np.percentile(x, 99.8), x.max(), 5.0))

    # The model uses fractional PD. Convert only the displayed x-axis to percent.
    xs = np.linspace(0.1, xhi, 600) / 100.0
    curve_y = K / (xs ** a)
    right_curve = float(curve_y[-1])
    yhi = min(
        max(float(np.percentile(yv, 99.5)), right_curve * 1.25, 2.0),
        12.0,
    )

    ax.scatter(
        x[normal], yv[normal], s=22, c="#22a447", alpha=0.58,
        linewidths=0, label=f"normal (below boundary, n={int(normal.sum()):,})",
        zorder=2,
    )
    ax.scatter(
        x[flagged], yv[flagged], s=38, facecolors="#ffedd5",
        edgecolors="#c2410c", alpha=0.82, linewidths=1.0,
        label=f"RS-flagged (above boundary, n={int(flagged.sum()):,})",
        zorder=3,
    )
    ax.scatter(x[bad], yv[bad], s=55, c=CLR["pd"], alpha=0.85, linewidths=0,
               label="distress — real credit event ≤ 3m", zorder=5)
    ax.plot(
        xs * 100, curve_y, color="#ea580c", lw=3.2, zorder=6,
        label=f"Hyperbolic boundary (K={K:.3f}, α={a:.2f})",
    )
    ax.fill_between(
        xs * 100, curve_y, yhi, where=curve_y <= yhi,
        color="#f97316", alpha=0.10, interpolate=True,
        label="flagged risk zone",
    )

    # A fixed lower limit of 0.4 hid valid boundaries such as K=0.14.
    ax.set_xlim(0, xhi); ax.set_ylim(0.0, yhi)
...
def fig_firm_trajectory(df, firm_val, pd_threshold=None):
    ...
    pd3m = pd.to_numeric(sub["PD_3M"], errors="coerce").values * 100
    haz = None
    if "h" in sub.columns:
        hazard_values = pd.to_numeric(sub["h"], errors="coerce")
        if hazard_values.notna().any():
            haz = hazard_values.values * 100
    ...
    if haz is not None:
        ax1.plot(
            t, haz, color=CLR["haz"], lw=1.6, ls="--",
            label="Monthly hazard h(t) (%)",
        )
    threshold = pd.to_numeric(
        pd.Series([pd_threshold]), errors="coerce"
    ).iloc[0]
    if pd.notna(threshold):
        threshold_percent = float(threshold) * 100
        ax1.axhline(
            threshold_percent, color=CLR["elev"], ls=":", lw=1.2,
            label=f"PD signal threshold {threshold_percent:.3g}%",
        )
    else:
        ax1.axhline(
            15, color=CLR["elev"], ls=":", lw=1.2,
            label="Elevated-risk level 15%",
        )
    ...
    momentum = (
        pd.to_numeric(sub["Momentum"], errors="coerce")
        .replace([np.inf, -np.inf], np.nan)
        .values
        if "Momentum" in sub.columns else None
    )

    def set_momentum_scale(axis):
        if momentum is None:
            return
        finite = momentum[np.isfinite(momentum)]
        if finite.size and np.nanmax(np.abs(finite)) > 20:
            axis.set_yscale("symlog", linthresh=1.0)
            axis.set_ylim(bottom=0)

    ax2t = None
    if dtd_col is not None and momentum is not None:
        ax2t = ax2.twinx()
        ax2t.spines["right"].set_visible(True)
        ax2t.grid(False)
        ax2t.plot(
            t, momentum, color=CLR["mom"], lw=1.8, ls="-.",
            marker="^", ms=3, label="Risk Momentum M(t)",
        )
        ...
        set_momentum_scale(ax2t)
    elif dtd_col is None and momentum is not None:
        ax2.clear()
        ax2.plot(
            t, momentum, color=CLR["mom"], lw=1.9, marker="^", ms=3.2,
            label="Risk Momentum M(t)",
        )
        ax2.axhline(1.0, color=CLR["mom"], ls=":", lw=0.9, alpha=0.6)
        ax2.set_ylabel("Momentum M(t)", color=CLR["mom"])
        ax2.tick_params(axis="y", colors=CLR["mom"])
        set_momentum_scale(ax2)
    if dmonth is not None:
        ax2.axvline(dmonth, color=CLR["risk"], ls="-", lw=1, alpha=0.35)
    ax2.set_xlabel("Month index")
    ax2.set_title(
        f"{fname} — DTD decay & risk acceleration"
        if dtd_col is not None
        else f"{fname} — risk momentum"
    )
    h1, l1 = ax2.get_legend_handles_labels()
    h2, l2 = (
        ax2t.get_legend_handles_labels() if ax2t is not None else ([], [])
    )
...
def firm_hazard_survival_frame(df, firm_val):
    """Return one firm's fitted monthly hazard and discrete survival path."""
    acct_col = (
        "firm_id"
        if "firm_id" in df.columns
        else ("ticker" if "ticker" in df.columns else "account_id")
    )
    sub = (
        df[df[acct_col].astype(str) == str(firm_val)]
        .sort_values("month_index")
        .copy()
    )
    if sub.empty or "h" not in sub.columns:
        return sub

    hazard = pd.to_numeric(sub["h"], errors="coerce").clip(0.0, 1.0)
    sub["_hazard_plot"] = hazard
    # For a discrete-time hazard, S(t) = product_j<=t (1 - h_j).
    sub["_survival_plot"] = np.cumprod(
        1.0 - hazard.fillna(0.0).to_numpy(dtype=float, copy=True)
    )
    return sub


def fig_firm_hazard_survival(df, firm_val):
    """Plot fitted h(t) and the implied S(t) for a selected Survivor2 firm."""
    sub = firm_hazard_survival_frame(df, firm_val)
    fname = _fmt_firm(firm_val)
    if sub.empty or "_hazard_plot" not in sub.columns:
        fig, ax = plt.subplots(figsize=(9.8, 3.3))
        ax.axis("off")
        ax.text(
            0.5,
            0.5,
            f"No fitted hazard data for {fname}",
            ha="center",
            va="center",
            color="#94a3b8",
        )
    ...
    x = pd.to_numeric(sub["month_index"], errors="coerce").to_numpy(copy=True)
    hazard_pct = sub["_hazard_plot"].to_numpy(dtype=float, copy=True) * 100.0
    survival_pct = sub["_survival_plot"].to_numpy(dtype=float, copy=True) * 100.0
    event_month = None
    if "event" in sub.columns:
        events = sub[pd.to_numeric(sub["event"], errors="coerce").fillna(0).gt(0)]
        if not events.empty:
            event_month = float(events["month_index"].iloc[0])

    fig, (ax_h, ax_s) = plt.subplots(1, 2, figsize=(11.0, 4.0), sharex=True)

    ax_h.fill_between(x, hazard_pct, color=CLR["haz"], alpha=0.12)
    ax_h.plot(
        x,
        hazard_pct,
        color=CLR["haz"],
    ...
    if "PD_3M" in sub.columns:
        pd3m = (
            pd.to_numeric(sub["PD_3M"], errors="coerce")
    ...
    ax_h.set_title(f"{fname} - fitted hazard")
    ax_h.set_xlabel("Month index")
    ax_h.set_ylabel("Probability (%)")
    ax_h.set_ylim(bottom=0)

    ax_s.fill_between(x, survival_pct, 100.0, color=CLR["dtd"], alpha=0.10)
    ax_s.plot(
        x,
        survival_pct,
        color=CLR["dtd"],
    ...
    ax_s.plot(
        ...
        lw=1.3,
        ls=":",
        label="Cumulative event risk 1-S(t) (%)",
    )
    ax_s.set_title(f"{fname} - implied survival probability")
    ax_s.set_xlabel("Month index")
    ax_s.set_ylabel("Probability (%)")
    ax_s.set_ylim(0, 105)

    if event_month is not None:
        for axis in (ax_h, ax_s):
            axis.axvline(
                event_month,
                color=CLR["risk"],
                lw=1.2,
                alpha=0.65,
                label="Observed credit event",
            )

    for axis in (ax_h, ax_s):
        axis.legend(fontsize=7.5, framealpha=0.92, loc="best")
        axis.margins(x=0.02)
    fig.suptitle(
        "Survivor2 firm path: discrete hazard h(t|X) and "
        "S(t) = product(1 - h(j))",
        fontsize=10.5,
        y=1.02,
    )
...
def fig_hazard(df, top_firms=6):
    ...
    Left   : mean/median fitted issuer hazard per month + credit events
    ...
    # --- 1. cross-sectional fitted hazard over time -----------------------
    ...
    ax1.set_title("Mean fitted issuer hazard over time")
    ...
    fig.suptitle("Stage 1 — fitted discrete-time hazard  h(t|X) = sigmoid(time(t) + β·X(t))",
...
def main(page):
    ...
    btn_tab20 = make_nav_button("Model Zoo: CatBoost / LGBM / RF / Logistic", ft.Icons.SCATTER_PLOT, 20)
    btn_tab21 = make_nav_button("Firm Shock & PD Threshold", ft.Icons.CRISIS_ALERT, 20)
    btn_tab21.tab_idx = 99
    ...
    surv_img = ft.Image(src="", visible=False, fit="contain", width=1100)
    ...
    def show_traj(firm_val):
        if state.get("df_surv") is None or firm_val in (None, ""):
            return
        try:
            meta = state.get("df_surv_meta") or {}
            b64_t = fig_firm_trajectory(
                state["df_surv"], firm_val, meta.get("tau_pd")
            )
            ...
            fname = _fmt_firm(firm_val)
            identity = (
                f"{fname} [source ID: {firm_val}]"
                if str(fname) != str(firm_val) else str(fname)
            )
            traj_caption.value = f"📉 Showing behaviour trajectory for {identity}"
        except Exception as ex:
    ...
    def on_survival(_):
        status.value = "Running Approach 1 Dynamic Survival EWS (Discrete-Time Hazard + Momentum + Hyperbolic Boundary)..."; page.update()
        try:
            use_ibond_33 = _is_ibond_33_table(state.get("active_table"))
            panel = load_panel(prefer_33=use_ibond_33)
            if panel is None or panel.empty:
                if not os.path.exists(PANEL_REAL):
                    import_real_to_sqlite()
                panel = load_panel(prefer_33=False)
    ...
    def on_run_survivor2(_):
        # Keep the historical button name, but run the complete visual EWS.
        # The old path populated only the lead-time table, leaving both chart
        # panels blank and making the two Survivor2 buttons behave differently.
        on_survivor2_ews(_)
    ...
    # --- Actions Toolbar ---
    def btn(txt, icon, fn, _color=None, visible=True):
        return ft.Button(
            txt,
            icon=icon,
            on_click=fn,
    ...
    toolbar = ft.ListView(controls=[
        btn("Import synthetic", ft.Icons.STORAGE, lambda _: run_async(on_import, "Importing synthetic data"), C.BLUE_600, visible=False),
        btn("Load REAL SET/US", ft.Icons.CLOUD_DOWNLOAD, lambda _: run_async(on_real, "Fetching REAL SET/US data"), C.TEAL_700, visible=False),
        ...
        btn("Run Survivor2 Engine", ft.Icons.MONITOR_HEART, lambda _: run_async(on_run_survivor2, "Running Survivor2 EWS"), C.PURPLE_800),
    ...
    s2ews_hazard_img = ft.Image(
        src="",
        visible=False,
        fit="contain",
        width=1100,
        height=340,
        error_content=ft.Text("The Survivor2 hazard chart could not be rendered."),
    )
    s2ews_hazard_note = ft.Text(
        "Click “Run Survivor2 Engine” to fit the hazard on the survivor2 panel "
    ...
    s2ews_box = ft.Column([])
    s2ews_boundary_img = ft.Image(
        src="",
        visible=False,
        fit="contain",
        width=1100,
        height=650,
        error_content=ft.Text("The Survivor2 hyperbolic chart could not be rendered."),
    )
    s2ews_firm_dropdown = ft.Dropdown(
        label="Select bond / firm",
        width=600,
        text_size=12,
        options=[],
    )
    s2ews_firm_caption = ft.Text(
        "", size=11, color=C.PURPLE_800, weight=ft.FontWeight.BOLD
    )
    s2ews_firm_chart_holder = ft.Column([
        ft.Container(
            content=ft.Text(
                "Firm-level hazard and survival probability will appear after the model run.",
                size=11,
                color=C.GREY_500,
            ),
            height=80,
    ...
    def show_s2_firm_chart(firm_val):
        if state.get("df_surv2") is None or firm_val in (None, ""):
            return
        try:
            chart_b64 = fig_firm_hazard_survival(state["df_surv2"], firm_val)
            s2ews_firm_chart_holder.controls = [
                ft.Image(
                    src=_uri(chart_b64),
                    visible=True,
                    fit="contain",
                    ...
                    error_content=ft.Text(
                        "The selected firm's hazard and survival chart could not be rendered."
                    ),
                )
            ]
            firm_name = _fmt_firm(firm_val)
            identity = (
                f"{firm_name} [source ID: {firm_val}]"
                if str(firm_name) != str(firm_val)
                else str(firm_name)
            )
            s2ews_firm_caption.value = (
                f"Showing fitted monthly hazard h(t) and survival S(t) for {identity}"
            )
        except Exception as ex:
            s2ews_firm_chart_holder.controls = [
                ft.Text(
                    f"Could not plot firm {firm_val}: {ex}",
                    color=C.RED_600,
                )
            ]
        page.update()

    def on_select_s2_firm(e):
        firm_val = s2ews_firm_dropdown.value
        if firm_val in (None, ""):
            firm_val = getattr(e, "data", None)
        show_s2_firm_chart(firm_val)

    s2ews_firm_dropdown.on_select = on_select_s2_firm
    s2ews_firm_dropdown.on_change = on_select_s2_firm

    def on_survivor2_ews(_):
        """Same output as 'Run Survival EWS (App 1)' but computed with survivor2.py / 33 features
        ...
        try:
            ...
            df_surv, meta = survival.run(
                panel.drop(columns=s2.HAZARD_DROP, errors="ignore")
            )
            state["df_surv2"] = df_surv
            state["df_surv2_meta"] = meta

            # Keep the already-mounted controls and update their data-URI src,
            # matching Approach 1 and both old/new Flet Image APIs.
            s2ews_hazard_img.src = _uri(fig_hazard(df_surv))
            s2ews_hazard_img.visible = True
            ...
            # Defaults first, then the riskiest currently monitored firms.
            event_firms = set()
            if "event" in df_surv.columns:
                event_firms = set(
                    df_surv.loc[
                        pd.to_numeric(
                            df_surv["event"], errors="coerce"
                        ).fillna(0).gt(0),
                        acct_col,
                    ].astype(str)
                )
            rank = {"HIGH RISK": 0, "ELEVATED": 1, "WATCH": 2, "OK": 3}
            firm_choices = latest.copy()
            firm_choices["_event_seen"] = (
                firm_choices[acct_col].astype(str).isin(event_firms)
            )
            firm_choices["_risk_rank"] = (
                firm_choices["status_text"].map(rank).fillna(4)
            )
            firm_choices = firm_choices.sort_values(
                ["_event_seen", "_risk_rank", "PD_3M", "h"],
                ascending=[False, True, False, False],
            ).head(80)
            s2ews_firm_dropdown.options = []
            for _, firm_row in firm_choices.iterrows():
                firm_id = firm_row[acct_col]
                event_label = (
                    "DEFAULT"
                    if str(firm_id) in event_firms
                    else firm_row["status_text"]
                )
                s2ews_firm_dropdown.options.append(
                    ft.dropdown.Option(
                        key=str(firm_id),
                        text=(
                            f"{_fmt_firm(firm_id)} - {event_label} - "
                            f"PD3M {float(firm_row['PD_3M']) * 100:.1f}% - "
                            f"h(t) {float(firm_row['h']) * 100:.2f}%"
                        ),
                    )
                )
            if s2ews_firm_dropdown.options:
                selected_firm = s2ews_firm_dropdown.options[0].key
                s2ews_firm_dropdown.value = selected_firm
                show_s2_firm_chart(selected_firm)
    ...
    s2ews_run_row = ft.Row([
        ft.Button("Run Survivor2 Engine", icon=ft.Icons.MONITOR_HEART,
        ...
        ft.Text("hazard plot on top → EWS metrics → lead-time DataGridView "
                "(survivor2.py panel with the shared discrete-time hazard engine)",
                size=11, color=C.GREY_600),
    ...
    s2ews_hazard_card = card(ft.Column([
        ...
        s2ews_hazard_note,
        s2ews_hazard_img,
        ft.Divider(height=1, color=C.PURPLE_100),
        ft.Row([
            ft.Icon(ft.Icons.QUERY_STATS, color=C.PURPLE_700, size=20),
            ft.Text(
                "Firm Hazard h(t) and Survival Probability S(t)",
                size=14,
                weight=ft.FontWeight.BOLD,
                color=C.BLUE_900,
            ),
    ...
    # ============ Model zoo: CatBoost / LightGBM / RF / Logistic =============
    zoo_info = ft.Text("Runs CatBoost, LightGBM, Random Forest and Logistic "
                       "Regression on the real iBond panel and reports how far each "
                       "one beats the Approach-1 baseline.", size=12, color=C.GREY_700)
    zoo_kpis = ft.Row([], spacing=10, wrap=True)
    zoo_table = ft.Column([])
    zoo_a2_table = ft.Column([])
    zoo_img = ft.Image(src="", visible=False, width=1120)
    zoo_log_lines = []
    zoo_log_text = ft.Text("", size=10.5, font_family="Consolas", selectable=True,
                           color=C.GREY_100)
    zoo_log_box = ft.Container(
        content=ft.Column([zoo_log_text], scroll=ft.ScrollMode.AUTO, auto_scroll=True),
        bgcolor="#0f172a", padding=10, border_radius=8, height=170,
        border=ft.Border.all(1, C.BLUE_GREY_700), visible=False)

    ZOO_C = {"XGBoost": C.BLUE_800, "CatBoost": C.DEEP_ORANGE_800,
             "LightGBM": C.AMBER_800, "Random Forest": C.GREEN_700,
             "Logistic (Approach 1)": C.GREY_600}

    def zoo_log(msg):
        zoo_log_lines.append(f"{pd.Timestamp.now():%H:%M:%S}  {msg}")
        del zoo_log_lines[:-400]
        zoo_log_text.value = "\n".join(zoo_log_lines)
        try:
            page.update()
        except Exception:
    ...
    def render_zoo_table(d, base_auc, base_f1):
        if d is None or d.empty:
            return ft.Text("no run yet", size=11, color=C.GREY_600)
        rows = []
        for _, r in d.iterrows():
            m = str(r["model"])
            is_base = m == "Logistic (Approach 1)"
            ap = (r["auc_oos"] - base_auc) / abs(base_auc) * 100 if base_auc else 0
            fp = (r["f1"] - base_f1) / abs(base_f1) * 100 if base_f1 else 0

            def pct(v, base):
                if base:
                    return ft.Text("baseline", size=10.5, color=C.GREY_600,
                                   italic=True)
                return ft.Text(f"{v:+.1f}%", size=11, weight=ft.FontWeight.BOLD,
                               color=C.GREEN_700 if v > 0 else
                               (C.RED_600 if v < 0 else C.GREY_600))
            rows.append(ft.DataRow(cells=[
                ft.DataCell(ft.Text(m, size=11, weight=ft.FontWeight.BOLD,
                                    color=ZOO_C.get(m, C.GREY_800))),
                ft.DataCell(ft.Text(f"{r['auc_in']:.4f}", size=11, color=C.GREY_600)),
                ft.DataCell(ft.Text(f"{r['auc_oos']:.4f}", size=11,
                                    weight=ft.FontWeight.BOLD)),
                ft.DataCell(ft.Text(f"{r['f1']:.4f}", size=11)),
                ft.DataCell(ft.Text(f"{r['recall']:.3f}", size=11)),
                ft.DataCell(ft.Text(f"{r['precision']:.3f}", size=11)),
                ft.DataCell(pct(ap, is_base)),
                ft.DataCell(pct(fp, is_base)),
            ]))
        return ft.Row([ft.DataTable(
            columns=[ft.DataColumn(ft.Text(c, size=11, weight=ft.FontWeight.BOLD))
                     for c in ["Model", "AUC in", "AUC oos", "F1", "Recall",
                               "Precision", "AUC vs A1", "F1 vs A1"]],
            rows=rows, column_spacing=16, data_row_min_height=30)],
            scroll=ft.ScrollMode.AUTO)

    def render_zoo_a2(d):
        if d is None or d.empty:
            return ft.Text("Approach-2 pipeline not run yet", size=11,
                           color=C.GREY_600)
        rows = []
        for _, r in d.iterrows():
            rows.append(ft.DataRow(cells=[
                ft.DataCell(ft.Text(str(r["learner"]), size=11,
                                    weight=ft.FontWeight.BOLD,
                                    color=ZOO_C.get(str(r["learner"]), C.GREY_800))),
                ft.DataCell(ft.Text(f"{r['auc_oos']:.4f}", size=11,
                                    weight=ft.FontWeight.BOLD,
                                    color=C.RED_600 if r["auc_oos"] < 0.75
                                    else C.GREY_900)),
                ft.DataCell(ft.Text(f"{r['brier_oos']:.5f}", size=11)),
                ft.DataCell(ft.Text(f"{int(r['n_caught'])}/{int(r['n_events'])}",
                                    size=11)),
                ft.DataCell(ft.Text(f"{r['persistent_median']:.0f} d", size=11)),
                ft.DataCell(ft.Text(f"{r['pct_high']:.2f}%", size=11)),
            ]))
        return ft.Row([ft.DataTable(
            columns=[ft.DataColumn(ft.Text(c, size=11, weight=ft.FontWeight.BOLD))
                     for c in ["Base learner", "AUC oos", "Brier", "Caught",
                               "Lead (persistent)", "HIGH RISK"]],
            rows=rows, column_spacing=18, data_row_min_height=28)],
            scroll=ft.ScrollMode.AUTO)

    def update_zoo_tab(_=None):
        con = sqlite3.connect(DB)
        try:
            cls = pd.read_sql("SELECT * FROM cmdf_classify_metrics", con)
        except Exception:
            cls = pd.DataFrame()
        try:
            a2 = pd.read_sql("SELECT * FROM cmdf_a2_compare", con)
        except Exception:
            a2 = pd.DataFrame()
        con.close()

        if cls.empty:
            zoo_info.value = ("No run saved yet — click “Run all models” to fit "
                              "CatBoost, LightGBM, Random Forest and Logistic on the "
                              "real iBond panel.")
            zoo_info.color = C.GREY_600
            page.update(); return

        base = cls[cls["model"] == "Logistic (Approach 1)"]
        b_auc = float(base.iloc[0]["auc_oos"]) if not base.empty else np.nan
        b_f1 = float(base.iloc[0]["f1"]) if not base.empty else np.nan
        d = cls.sort_values("auc_oos", ascending=False)
        ...
        def kpi(t, v, s, ic, c, bg):
            return ft.Container(content=ft.Row([
                ...
                border=ft.Border.all(1, c), width=212, shadow=SHADOW)

        best_a = d.iloc[0]
        best_f = d.loc[d["f1"].idxmax()]
        n_beat = int((d["auc_oos"] > b_auc).sum()) - 0
        zoo_kpis.controls = [
            kpi("🥇 Best AUC", f"{best_a['auc_oos']:.4f}", str(best_a["model"]),
                ft.Icons.EMOJI_EVENTS, C.BLUE_800, C.BLUE_50),
            kpi("🎯 Best F1", f"{best_f['f1']:.4f}", str(best_f["model"]),
                ft.Icons.GPS_FIXED, C.GREEN_700, C.GREEN_50),
            kpi("📐 Approach-1 baseline", f"{b_auc:.4f}", f"F1 {b_f1:.4f}",
                ft.Icons.STRAIGHTEN, C.GREY_700, C.GREY_100),
            kpi("📊 Beat the baseline", f"{n_beat} / {len(d) - 1}",
                "on out-of-sample AUC", ft.Icons.TRENDING_UP,
                C.TEAL_700, C.TEAL_50),
            kpi("🔬 Events", f"{int(d.iloc[0]['n_pos'])}",
                f"of {int(d.iloc[0]['n_eval']):,} held-out rows",
                ft.Icons.SCIENCE, C.PURPLE_700, C.PURPLE_50),
        ]
        zoo_table.controls = [
            ft.Text("Table 1 — all models against the Approach-1 baseline", size=12,
                    weight=ft.FontWeight.BOLD, color=C.BLUE_900),
            ft.Text("Leave-one-issuer-out on the real iBond panel. F1, recall and "
                    "precision use a matched alarm budget of 2% of issuer-months, so "
                    "every model flags the same number of cases. AUC in-sample is "
                    "shown only to expose the overfitting gap.",
                    size=10, color=C.GREY_600),
            render_zoo_table(d, b_auc, b_f1)]
        zoo_a2_table.controls = [
            ft.Text("Table 2 — Approach-2 calibrated pipeline, four base learners",
                    size=12, weight=ft.FontWeight.BOLD, color=C.BLUE_900),
            render_zoo_a2(a2)] if not a2.empty else []
        try:
            p = os.path.join(DATA_ROOT, "tex_out", "fig_classify_compare.png")
            if os.path.exists(p):
                import base64 as _b64
                with open(p, "rb") as f:
                    zoo_img.src = _uri(_b64.b64encode(f.read()).decode())
                zoo_img.visible = True
        except Exception:
            pass
        zoo_info.value = (f"Real iBond panel · {int(d.iloc[0]['n_eval']):,} held-out "
                          f"rows · {int(d.iloc[0]['n_pos'])} default events · "
                          f"SQLite: cmdf_classify_metrics / cmdf_a2_compare")
        zoo_info.color = C.TEAL_800
        page.update()

    def on_zoo_run(which):
        def _worker(_):
            zoo_log_lines.clear()
            zoo_log_box.visible = True
            zoo_info.value = "Fitting models on the real iBond panel …"
            zoo_info.color = C.ORANGE_800
            status.value = "Model zoo running …"
            page.update()
            ...
            try:
                t0 = pd.Timestamp.now()
                if which in ("all", "classify"):
                    import cmdf_tree_classify as ctc
                    zoo_log("running the four tree models plus the logistic baseline")
                    with contextlib.redirect_stdout(_Tee()):
                        ctc.run(save=True, verbose=True)
                if which in ("all", "a2"):
                    import cmdf_approach2_compare as a2c
                    zoo_log("running the Approach-2 calibrated pipeline")
                    with contextlib.redirect_stdout(_Tee()):
                        a2c.run(save=True, verbose=True)
                secs = (pd.Timestamp.now() - t0).total_seconds()
                zoo_log(f"finished in {secs:.1f}s")
                update_zoo_tab()
                status.value = f"Model zoo done in {secs:.0f}s"
            except Exception as ex:
                zoo_log(f"ERROR: {ex}")
                zoo_info.value = f"Run failed: {ex}"
                zoo_info.color = C.RED_600
                status.value = f"Model zoo error: {ex}"
            page.update()

        run_async(_worker, "Model zoo")

    zoo_card = card(ft.Column([
        ft.Row([ft.Icon(ft.Icons.SCATTER_PLOT, color=C.DEEP_ORANGE_800, size=22),
                ft.Text("🧪 Model zoo — CatBoost / LightGBM / Random Forest / Logistic",
                        size=15, weight=ft.FontWeight.BOLD, color=C.BLUE_900)],
               spacing=6),
        ft.Text("Fits every learner on the real iBond 33-determinant panel with the "
                "real payment-default register as the target, validates by holding out "
                "one defaulted issuer at a time, and reports the percentage "
                "improvement over the Approach-1 logistic baseline.",
                size=11, color=C.GREY_700),
        ft.Row([
            ft.Button("Run all models", icon=ft.Icons.PLAY_ARROW,
                      on_click=lambda _: on_zoo_run("all"),
                      bgcolor=C.DEEP_ORANGE_800, color=C.WHITE,
                      style=ft.ButtonStyle(shape=ft.RoundedRectangleBorder(radius=8))),
            ft.Button("Classification only", icon=ft.Icons.CATEGORY,
                      on_click=lambda _: on_zoo_run("classify"),
                      bgcolor=C.BLUE_700, color=C.WHITE,
                      style=ft.ButtonStyle(shape=ft.RoundedRectangleBorder(radius=8))),
            ft.Button("Approach-2 pipeline only", icon=ft.Icons.TIMELINE,
                      on_click=lambda _: on_zoo_run("a2"),
                      bgcolor=C.INDIGO_700, color=C.WHITE,
                      style=ft.ButtonStyle(shape=ft.RoundedRectangleBorder(radius=8))),
            ft.Button("Reload saved", icon=ft.Icons.REFRESH, on_click=update_zoo_tab,
                      bgcolor=C.BLUE_GREY_600, color=C.WHITE,
                      style=ft.ButtonStyle(shape=ft.RoundedRectangleBorder(radius=8))),
        ], spacing=8, wrap=True),
        zoo_info,
        ...
        zoo_a2_table,
        zoo_img,
    ], spacing=12), accent=C.DEEP_ORANGE_200, pad=14)

    view_zoo = ft.Container(
        content=ft.Column([zoo_card,
    ...
    fs_status = ft.Text("", size=12, color=C.BLUE_GREY_700)
    fs_kpis = ft.Row(spacing=10, wrap=True)
    fs_images = ft.Column(spacing=12)
    fs_ladder_host = ft.Column(spacing=6)
    fs_table_host = ft.Column(spacing=6)

    fs_issuer_dd = ft.Dropdown(label="Issuer", width=210, options=[], dense=True)
    fs_page_label = ft.Text("", size=12, color=C.BLUE_GREY_700)
    fs_filter_dd = ft.Dropdown(
        label="Show", width=170, dense=True, value="ALL",
        options=[ft.DropdownOption(key="ALL", text="all issuers"),
                 ft.DropdownOption(key="HIGH RISK", text="HIGH RISK only"),
                 ft.DropdownOption(key="WATCH", text="WATCH only"),
                 ft.DropdownOption(key="OK", text="OK only")])
    fs_size_dd = ft.Dropdown(
        label="Rows", width=110, dense=True, value="25",
    ...
    fs_workload_dd = ft.Dropdown(
        label="Review capacity", width=190, dense=True,
        options=[ft.DropdownOption(key="0.02", text="2% of issuer-months"),
                 ft.DropdownOption(key="0.05", text="5%  (recommended)"),
                 ft.DropdownOption(key="0.10", text="10% (3 staff, 30 names)")],
        value="0.05")

    def _fs_kpi(label, value, tone="#1d4ed8"):
        return ft.Container(
            content=ft.Column([
            ...
            border=ft.Border.all(1, UI["border"]), width=168)

    def _fs_visible_rows():
        """The table after the status filter, before paging."""
        d = fs_state.get("table")
        if d is None or d.empty:
            return d
        f = fs_state.get("filter", "ALL")
        return d if f == "ALL" else d[d.status == f]

    def _fs_render_table():
        d = _fs_visible_rows()
        fs_table_host.controls.clear()
        if d is None or d.empty:
            fs_page_label.value = "no issuers match this filter"
            return
        size = int(fs_state.get("page_size", 25))
        total = len(d)
        max_page = max(0, (total - 1) // size)
        fs_state["page"] = min(max(int(fs_state.get("page", 0)), 0), max_page)
        off = fs_state["page"] * size
        head = d.iloc[off:off + size]
        fs_page_label.value = (f"page {fs_state['page']+1} of {max_page+1}   |   "
                               f"rows {off+1}-{min(off+size, total)} of {total:,}")
        rows = []
        for _, r in head.iterrows():
            tone = (C.RED_700 if r.status == "HIGH RISK"
                    else (C.ORANGE_800 if r.status == "WATCH" else C.GREEN_800))
            rows.append(ft.DataRow([
                ft.DataCell(ft.Text(str(r.issuer), weight=ft.FontWeight.BOLD)),
                ft.DataCell(ft.Text(str(r.month))),
                ft.DataCell(ft.Text(f"{r.pd_now:.6f}")),
                ft.DataCell(ft.Text(f"{r.percentile:.2f}")),
                ft.DataCell(ft.Text(f"{r.margin:+.6f}")),
                ft.DataCell(ft.Text(r.status, color=tone,
                                    weight=ft.FontWeight.BOLD)),
            ]))
        fs_table_host.controls.append(scrollable_data_table(ft.DataTable(
            columns=[ft.DataColumn(ft.Text("Issuer", weight=ft.FontWeight.BOLD)),
                     ft.DataColumn(ft.Text("Month", weight=ft.FontWeight.BOLD)),
                     ft.DataColumn(ft.Text("PD (out-of-fold)",
                                           weight=ft.FontWeight.BOLD)),
                     ft.DataColumn(ft.Text("Percentile", weight=ft.FontWeight.BOLD)),
                     ft.DataColumn(ft.Text("Margin", weight=ft.FontWeight.BOLD)),
                     ft.DataColumn(ft.Text("Status", weight=ft.FontWeight.BOLD))],
            rows=rows, heading_row_height=34, data_row_max_height=32)))

    def _fs_render_ladder(lad):
        fs_ladder_host.controls.clear()
        if lad is None or lad.empty:
            return
        rows = []
        for _, r in lad.iterrows():
            reach = bool(r.reachable)
            rows.append(ft.DataRow([
                ft.DataCell(ft.Text(str(r.feature))),
                ft.DataCell(ft.Text(str(r.direction))),
                ft.DataCell(ft.Text(f"{r.current:.4f}")),
                ft.DataCell(ft.Text("--" if not reach else f"{r.sd_needed:.2f}",
                                    weight=ft.FontWeight.BOLD,
                                    color=C.RED_700 if reach else C.BLUE_GREY_400)),
                ft.DataCell(ft.Text("--" if not reach else f"{r.value_needed:.4f}")),
                ft.DataCell(ft.Text("yes" if reach else "cannot alone",
                                    color=C.GREEN_800 if reach else C.BLUE_GREY_500)),
            ]))
        fs_ladder_host.controls.append(scrollable_data_table(ft.DataTable(
            columns=[ft.DataColumn(ft.Text("Determinant", weight=ft.FontWeight.BOLD)),
                     ft.DataColumn(ft.Text("Move", weight=ft.FontWeight.BOLD)),
                     ft.DataColumn(ft.Text("Current", weight=ft.FontWeight.BOLD)),
                     ft.DataColumn(ft.Text("SD needed", weight=ft.FontWeight.BOLD)),
                     ft.DataColumn(ft.Text("Value at line",
                                           weight=ft.FontWeight.BOLD)),
                     ft.DataColumn(ft.Text("Alone?", weight=ft.FontWeight.BOLD))],
            rows=rows, heading_row_height=34, data_row_max_height=32)))

    def _fs_load_issuer(issuer):
        fs_kpis.controls.clear(); fs_images.controls.clear()
        if not issuer:
            return
        fs_status.value = f"computing {issuer} ..."
        page.update()
        try:
            res = _fsp.build_panel_for_issuer(issuer, fs_state["workload"])
        except Exception as ex:
            fs_status.value = f"failed: {ex}"
            page.update(); return
        if res is None:
            fs_status.value = f"{issuer} has no rows in the panel"
            page.update(); return
        sm = res["summary"]
        tone = C.RED_700 if sm["breach"] else C.GREEN_800
        fs_kpis.controls.extend([
            _fs_kpi("PD, latest month", f"{sm['pd_now']:.6f}", tone),
            _fs_kpi("Percentile", f"{sm['percentile']:.2f}"),
            _fs_kpi("Threshold", f"{sm['threshold']:.6f}"),
            _fs_kpi("Margin", f"{sm['margin']:+.6f}", tone),
            _fs_kpi("Status", "HIGH RISK" if sm["breach"] else "OK", tone),
            _fs_kpi("Months observed", sm["n_months"]),
            _fs_kpi("Recorded event", "yes" if sm["event"] else "no",
                    C.RED_700 if sm["event"] else C.BLUE_GREY_700),
        ])
        for key, cap in (("path", "PD path against the threshold"),
                         ("pairs", "position against the boundary on four "
                                   "determinant pairs: two from the mean gain "
                                   "of all four models, two from XGBoost alone"),
                         ("ladder", "shock ladder")):
            b64 = res["figures"].get(key)
            if not b64:
                continue
        ...
        fs_status.value = (f"{issuer}: {sm['n_months']} months, "
                           f"{sm['first_month']} to {sm['last_month']}   |   "
                           f"mean gain: {', '.join(res['pairs_mean'])}   |   "
                           f"XGBoost gain: {', '.join(res['pairs_xgb'])}")
        page.update()

    def update_firmshock_tab(_=None):
        try:
            d = _fsp.issuer_table(fs_state["workload"])
        except Exception as ex:
            fs_status.value = f"cannot load the iBond panel: {ex}"
            page.update(); return
        fs_state["table"] = d
        fs_state["page"] = 0
        fs_issuer_dd.options = [ft.DropdownOption(key=str(v), text=str(v))
                                for v in d.issuer.tolist()]
        if fs_state["issuer"] not in set(d.issuer):
            fs_state["issuer"] = str(d.issuer.iloc[0])
        fs_issuer_dd.value = fs_state["issuer"]
        n_b = int((d.status == "HIGH RISK").sum())
        n_n = int((d.status == "WATCH").sum())
        fs_status.value = (f"{len(d)} issuers scored out of fold   |   "
                           f"{n_b} above the review threshold, {n_n} within two "
    ...
    def _fs_page(delta):
        ...
        if d is None or d.empty:
            return
        size = int(fs_state.get("page_size", 25))
        max_page = max(0, (len(d) - 1) // size)
        new_page = min(max(fs_state.get("page", 0) + delta, 0), max_page)
        if new_page == fs_state.get("page", 0):
            return
        fs_state["page"] = new_page
        _fs_render_table()
        page.update()

    def _fs_first(_):
        fs_state["page"] = 0
        _fs_render_table(); page.update()

    def _fs_last(_):
        d = _fs_visible_rows()
        if d is None or d.empty:
            return
        fs_state["page"] = max(0, (len(d) - 1) // int(fs_state["page_size"]))
        _fs_render_table(); page.update()

    def _fs_on_filter(e):
        fs_state["filter"] = fs_filter_dd.value
        fs_state["page"] = 0
        _fs_render_table(); page.update()

    def _fs_on_size(e):
        fs_state["page_size"] = int(fs_size_dd.value)
        fs_state["page"] = 0
        _fs_render_table(); page.update()
    ...
    fs_filter_dd.on_select = _fs_on_filter
    ...
    firmshock_card = ft.Container(
        content=ft.Column([
            ft.Text("Firm Shock & PD Threshold", size=18,
                    weight=ft.FontWeight.BOLD, color=C.BLUE_900),
            ft.Text("Every number on this tab comes from ibond_33features_panel. "
                    "Probabilities are out-of-fold under grouped cross-validation "
                    "over issuer identity, so an issuer's own months never trained "
                    "the model that scores it. The threshold is anchored on review "
                    "capacity rather than on an absolute probability, because the "
                    "Brier Skill Score is negative on this panel and an absolute "
                    "ceiling would not mean what it says. HIGH RISK means the issuer sits above that review threshold; it does not mean the issuer has defaulted.",
                    size=11, color=C.BLUE_GREY_700),
            ft.Row([fs_issuer_dd, fs_workload_dd,
                    ft.FilledButton("Refresh", icon=ft.Icons.REFRESH,
                                    on_click=lambda _: update_firmshock_tab())],
                   spacing=10, wrap=True),
            fs_status,
            ...
            ft.Text("All issuers, latest month", size=13,
                    weight=ft.FontWeight.BOLD, color=C.BLUE_900),
            ft.Row([
                fs_filter_dd, fs_size_dd,
                ft.IconButton(ft.Icons.FIRST_PAGE, tooltip="first page",
                              on_click=_fs_first),
                ft.IconButton(ft.Icons.CHEVRON_LEFT, tooltip="previous page",
                              on_click=lambda _: _fs_page(-1)),
                ft.IconButton(ft.Icons.CHEVRON_RIGHT, tooltip="next page",
                              on_click=lambda _: _fs_page(1)),
                ft.IconButton(ft.Icons.LAST_PAGE, tooltip="last page",
                              on_click=_fs_last),
                fs_page_label,
    ...
    view_firmshock = ft.Container(
        content=ft.Column([firmshock_card,
    ...
    # ============ Momentum & Hyperbolic Boundary (real iBond panel) ==========
    import hyperbolic_boundary_panel as _hbp

    hb_state = {"workload": 0.05, "issuer": None, "res": None}

    hb_status = ft.Text("", size=12, color=C.BLUE_GREY_700)
    hb_kpis = ft.Row(spacing=10, wrap=True)
    hb_images = ft.Column(spacing=12)
    hb_cmp_host = ft.Column(spacing=6)
    hb_event_host = ft.Column(spacing=6)

    hb_issuer_dd = ft.Dropdown(label="Trace issuer", width=210, options=[],
                               dense=True)
    hb_workload_dd = ft.Dropdown(
        label="Review capacity", width=190, dense=True,
        options=[ft.DropdownOption(key="0.02", text="2% of issuer-months"),
                 ft.DropdownOption(key="0.05", text="5%  (recommended)"),
    ...
    def _hb_kpi(label, value, tone="#1d4ed8"):
        return ft.Container(
            content=ft.Column([
                ft.Text(label, size=10, color=C.BLUE_GREY_600),
                ft.Text(str(value), size=16, weight=ft.FontWeight.BOLD, color=tone),
    ...
    def _hb_render_compare(res):
        hb_cmp_host.controls.clear()
        rows = []
        for nm, r in (("PD level only", res["level"]),
                      (f"boundary  a={res['alpha']:.2f}", res["boundary"])):
            rows.append(ft.DataRow([
                ft.DataCell(ft.Text(nm, weight=ft.FontWeight.BOLD)),
                ft.DataCell(ft.Text(str(int(r["tp"])))),
                ft.DataCell(ft.Text(f"{int(r['fp']):,}")),
                ft.DataCell(ft.Text(str(int(r["fn"])))),
                ft.DataCell(ft.Text(f"{r['recall']:.4f}")),
                ft.DataCell(ft.Text(f"{r['precision']:.4f}")),
                ft.DataCell(ft.Text(f"{r['mcc']:.4f}", weight=ft.FontWeight.BOLD)),
            ]))
        hb_cmp_host.controls.append(scrollable_data_table(ft.DataTable(
            columns=[ft.DataColumn(ft.Text("Rule", weight=ft.FontWeight.BOLD)),
                     ft.DataColumn(ft.Text("TP", weight=ft.FontWeight.BOLD)),
                     ft.DataColumn(ft.Text("FP", weight=ft.FontWeight.BOLD)),
                     ft.DataColumn(ft.Text("FN", weight=ft.FontWeight.BOLD)),
                     ft.DataColumn(ft.Text("Recall", weight=ft.FontWeight.BOLD)),
                     ft.DataColumn(ft.Text("Precision", weight=ft.FontWeight.BOLD)),
                     ft.DataColumn(ft.Text("MCC", weight=ft.FontWeight.BOLD))],
            rows=rows, heading_row_height=34, data_row_max_height=32)))

    def _hb_render_events(res):
        hb_event_host.controls.clear()
        d = res["events"]
        if d is None or d.empty:
            return
        rows = []
        for _, r in d.iterrows():
            alarm = str(r.side) == "ALARM"
            mom = "--" if not (r.momentum == r.momentum) else f"{r.momentum:.4f}"
            mar = "--" if not (r.margin == r.margin) else f"{r.margin:+.4f}"
            rows.append(ft.DataRow([
                ft.DataCell(ft.Text(str(r.issuer_code),
                                    weight=ft.FontWeight.BOLD)),
                ft.DataCell(ft.Text(str(r.month))),
                ft.DataCell(ft.Text(f"{r.pd_now:.6f}")),
                ft.DataCell(ft.Text(mom)),
                ft.DataCell(ft.Text(mar)),
                ft.DataCell(ft.Text(str(r.side),
                                    color=C.RED_700 if alarm else C.BLUE_GREY_500,
                                    weight=ft.FontWeight.BOLD)),
            ]))
        hb_event_host.controls.append(scrollable_data_table(ft.DataTable(
            columns=[ft.DataColumn(ft.Text("Issuer", weight=ft.FontWeight.BOLD)),
                     ft.DataColumn(ft.Text("Month", weight=ft.FontWeight.BOLD)),
                     ft.DataColumn(ft.Text("PD(t)", weight=ft.FontWeight.BOLD)),
                     ft.DataColumn(ft.Text("Momentum", weight=ft.FontWeight.BOLD)),
                     ft.DataColumn(ft.Text("Margin", weight=ft.FontWeight.BOLD)),
                     ft.DataColumn(ft.Text("Side", weight=ft.FontWeight.BOLD))],
            rows=rows, heading_row_height=34, data_row_max_height=32)))

    def update_hyperbolic_tab(_=None):
        hb_status.value = "computing the boundary ..."
        page.update()
        try:
            res = _hbp.build_panel(hb_state["workload"], hb_state["issuer"])
        except Exception as ex:
            hb_status.value = f"failed: {ex}"
            page.update(); return
        hb_state["res"] = res
        hb_kpis.controls.clear(); hb_images.controls.clear()

        gain = float(res["boundary"]["mcc"]) - float(res["level"]["mcc"])
        tone = C.GREEN_800 if gain > 0.01 else C.BLUE_GREY_700
        hb_kpis.controls.extend([
            _hb_kpi("alpha (in sample)", f"{res['alpha']:.2f}"),
            _hb_kpi("log K", f"{res['logK']:.4f}"),
            _hb_kpi("PD threshold", f"{res['threshold']:.6f}"),
            _hb_kpi("Usable rows", f"{res['n_usable']:,}"),
            _hb_kpi("Event months", res["n_events"]),
            _hb_kpi("MCC, boundary", f"{res['boundary']['mcc']:.4f}"),
            _hb_kpi("MCC gain over PD", f"{gain:+.4f}", tone),
        ])
        for key, cap in (
                ("boundary", "where every issuer-month sits relative to the "
                             "boundary; green crosses are the recorded event months"),
                ("alpha", "alpha is scanned at fixed workload; the dashed line is "
                          "the plain PD rule on the same rows")):
            b64 = res["figures"].get(key)
            if not b64:
                continue
        ...
        _hb_render_events(res)

        d = res["issuers"]
        hb_issuer_dd.options = [ft.DropdownOption(key=str(v), text=str(v))
                                for v in d.issuer_code.tolist()]
        if hb_state["issuer"] not in set(d.issuer_code):
            hb_state["issuer"] = None
        hb_issuer_dd.value = hb_state["issuer"]
        n_alarm = int((d.side == "ALARM").sum())
        ev = d[d.ever_event]
        hb_status.value = (
            f"{n_alarm} of {len(d)} issuers alarm on their latest month   |   "
            f"{int((ev.side == 'ALARM').sum())} of {len(ev)} issuers that ever "
            f"recorded an event alarm now   |   momentum is exactly 1.0000 for "
            f"{int((d.momentum.round(6) == 1.0).sum())} issuers, because the panel "
            f"carries the last observation forward")
        page.update()

    def _hb_on_issuer(e):
        hb_state["issuer"] = hb_issuer_dd.value
        update_hyperbolic_tab()

    def _hb_on_workload(e):
        hb_state["workload"] = float(hb_workload_dd.value)
        update_hyperbolic_tab()

    hb_issuer_dd.on_select = _hb_on_issuer
    hb_workload_dd.on_select = _hb_on_workload

    hyperbolic_card = ft.Container(
        content=ft.Column([
            ft.Text("Momentum & Hyperbolic Boundary", size=18,
                    weight=ft.FontWeight.BOLD, color=C.BLUE_900),
            ft.Text("The level rule asks whether PD is high now. This rule asks a "
                    "second question, whether PD is rising fast from where it was, "
                    "and combines the two: alarm when M(t) x PD(t-1)^alpha >= K, "
                    "where M(t) = PD(t)/PD(t-1). In the (PD, M) plane that is a "
                    "hyperbola. PD is out-of-fold, the same series the Firm Shock "
                    "tab uses, so the two tabs are on one scale. alpha is chosen on "
                    "the same 32 event months it is scored against, so the MCC gain "
                    "shown is optimistic and is not out-of-sample evidence.",
                    size=11, color=C.BLUE_GREY_700),
            ft.Row([hb_issuer_dd, hb_workload_dd,
                    ft.FilledButton("Compute", icon=ft.Icons.REFRESH,
                                    on_click=lambda _: update_hyperbolic_tab())],
                   spacing=10, wrap=True),
            hb_status,
            ft.Divider(height=1, color=UI["border"]),
            hb_kpis,
            ft.Text("Does the extra parameter buy anything? Both rules alarm on the "
                    "same share of rows.", size=13, weight=ft.FontWeight.BOLD,
                    color=C.BLUE_900),
            hb_cmp_host,
            hb_images,
            ft.Text("Every recorded event month: momentum and side of the boundary",
                    size=13, weight=ft.FontWeight.BOLD, color=C.BLUE_900),
            hb_event_host,
        ], spacing=12),
        padding=16, bgcolor=UI["surface"], border_radius=12,
    ...
    def update_datagrid_tab():
        # The lower inspector already contains the active dataset table.
        # Keeping data_grid_card in the optional top panel rendered the same
        # 33-feature DataGrid twice (one above the other).
        if top_panel_container.content is data_grid_card:
            top_panel_container.content = None
    ...
    def set_tab(idx):
        ...
        if idx >= 21:
            view, refresh = baseline_views[idx - 21]
            refresh()
        ...
        if idx == 0:
            ...
            update_zoo_tab()
            main_content_container.content = view_zoo
        else:
            main_content_container.content = view_bond_news
        page.update()
    ...
    btn_tab20.on_click = lambda _: set_tab(20)
    for _i, _b in enumerate(baseline_buttons, start=21):
    ...
    if globals().get("_UITEST"):
        ...
        for name, h in handlers:
            try:
                h(None)
                if name.startswith("on_alerts"):
                    assert not str(status.value).startswith(
                        "Error computing alerts:"
                    ), status.value
                    assert isinstance(state.get("alerts"), pd.DataFrame)
                    assert not state["alerts"].empty
                    assert dist_img.visible
                    assert str(dist_img.src).startswith(
                        "data:image/png;base64,"
                    )
                    print(
                        f"app2 alerts OK rows={len(state['alerts']):,} "
                        f"chart={dist_img.visible}"
                    )
            except Exception:
        ...
        try:
            ...
            # Reproduce Load bond 33-feat: the active table belongs only in the
            # lower inspector, not in both the optional top panel and inspector.
            top_panel_container.content = data_grid_card
            update_datagrid_tab()
            assert top_panel_container.content is None
            assert len(view_datagrid.controls) == 3
            print("33-feature datagrid dedup OK visible_inspectors=1")
        except Exception:
            print("DATAGRID SCROLLBAR ERROR"); traceback.print_exc()

        # Model zoo tab must render from the saved metric tables
        try:
            set_tab(20)
            print(f"model-zoo tab OK kpis={len(zoo_kpis.controls)} "
                  f"table={len(zoo_table.controls)>0} "
                  f"a2={len(zoo_a2_table.controls)>0} chart={zoo_img.visible}")
        except Exception:
            print("MODEL-ZOO TAB ERROR"); traceback.print_exc()

        # Firm shock tab: the issuer grid, the KPI row and all three figures must
        # come back from the real panel, and switching issuer must rebuild them
        try:
            set_tab(99)
            d = fs_state.get("table")
            n_b = int((d.status == "HIGH RISK").sum()) if d is not None else -1
            first = fs_state.get("issuer")
            n_img_1 = len(fs_images.controls)
            other = str(d.issuer.iloc[1]) if d is not None and len(d) > 1 else None
            if other:
                fs_issuer_dd.value = other
                _fs_on_issuer(None)
            pg0 = fs_page_label.value
            _fs_page(1); pg1 = fs_page_label.value
            _fs_last(None); pgL = fs_page_label.value
            fs_filter_dd.value = "HIGH RISK"; _fs_on_filter(None)
            n_hr = len(_fs_visible_rows())
            fs_size_dd.value = "10"; _fs_on_size(None)
            pg10 = fs_page_label.value
            fs_filter_dd.value = "ALL"; _fs_on_filter(None)
            fs_size_dd.value = "25"; _fs_on_size(None)
            print(f"firm-shock pager OK p1='{pg0}' p2='{pg1}' last='{pgL}' "
                  f"highrisk_rows={n_hr} size10='{pg10}'")
            print(f"firm-shock tab OK issuers={0 if d is None else len(d)} "
                  f"breach={n_b} kpis={len(fs_kpis.controls)} "
                  f"figures={n_img_1} ladder={len(fs_ladder_host.controls)>0} "
        ...
        # Momentum & hyperbolic boundary tab
        try:
            set_tab(98)
            r = hb_state.get("res")
            pos = r["issuers"]
            gain = float(r["boundary"]["mcc"]) - float(r["level"]["mcc"])
            print(f"hyperbolic tab OK alpha={r['alpha']:.2f} "
                  f"logK={r['logK']:.4f} usable={r['n_usable']} "
                  f"events={r['n_events']} "
                  f"mcc_boundary={r['boundary']['mcc']:.4f} "
                  f"mcc_level={r['level']['mcc']:.4f} gain={gain:+.4f} "
                  f"alarm_now={int((pos.side=='ALARM').sum())}/{len(pos)} "
                  f"kpis={len(hb_kpis.controls)} figures={len(hb_images.controls)} "
                  f"compare={len(hb_cmp_host.controls)>0} "
        ...
        if state.get("df_surv") is not None and firm_select_dropdown.options:
            app1_diag = boundary_plot_diagnostics(
                state["df_surv"], state["df_surv_meta"]
            )
            assert app1_diag["normal_rows"] > 0
            assert app1_diag["flagged_rows"] > 0
            assert "h" in state["df_surv"].columns
            print(
                f"app1 boundary OK normal={app1_diag['normal_rows']:,} "
                f"flagged={app1_diag['flagged_rows']:,} "
                f"K={state['df_surv_meta']['boundary']['K']:.3f}"
            )
        ...
        on_bond(None)
        assert state.get("active_table") == "v_ibond_33features_panel"
        _mounted_s2_boundary = s2ews_boundary_img
        _mounted_s2_hazard = s2ews_hazard_img
        # Exercise the exact user path: Load bond 33-feat -> Run Survivor2 Engine.
        on_run_survivor2(None)
        _s2_diag = boundary_plot_diagnostics(
            state["df_surv2"], state["df_surv2_meta"]
        )
        assert _s2_diag["normal_rows"] > 0
        assert _s2_diag["flagged_rows"] > 0
        assert pd.to_numeric(
            state["df_surv2"]["h"], errors="coerce"
        ).notna().any()
        assert s2ews_boundary_img is _mounted_s2_boundary
        assert s2ews_hazard_img is _mounted_s2_hazard
        assert str(s2ews_boundary_img.src).startswith("data:image/png;base64,")
        assert str(s2ews_hazard_img.src).startswith("data:image/png;base64,")
        assert len(s2ews_boundary_img.src) > 10_000
        assert len(s2ews_hazard_img.src) > 10_000
        assert s2ews_metrics_card.content.controls[1] is s2ews_boundary_img
        assert s2ews_hazard_card.content.controls[3] is s2ews_hazard_img
        assert len(s2ews_firm_dropdown.options) > 0
        assert s2ews_firm_dropdown.on_select is on_select_s2_firm
        assert s2ews_firm_dropdown.on_change is on_select_s2_firm
        selected_option = s2ews_firm_dropdown.options[-1]
        s2ews_firm_dropdown.value = selected_option.key
        on_select_s2_firm(
            type("Survivor2SelectEvent", (), {"data": selected_option.key})()
        )
        selected_chart = s2ews_firm_chart_holder.controls[0]
        assert str(selected_chart.src).startswith("data:image/png;base64,")
        assert len(selected_chart.src) > 10_000
        assert str(selected_option.key) in (s2ews_firm_caption.value or "")
        print(f"survivor2-EWS OK hazard_chart={s2ews_hazard_img.visible} "
              ...
              f"normal={_s2_diag['normal_rows']:,} "
              f"flagged={_s2_diag['flagged_rows']:,} "
              f"firm_dropdown={len(s2ews_firm_dropdown.options)} "
              f"button_path=engine mounted_images=True")
